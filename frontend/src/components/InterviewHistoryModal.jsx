...
import { X, Mic, TrendingUp } from 'lucide-react';
import axios from 'axios';

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000';

export default function InterviewHistoryModal({ isOpen, onClose }) {
        ...
        >
          <div className="flex justify-between items-center p-6 border-b border-black/5 bg-gray-50/50 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-gray-900 font-display">Interview History</h2>
            </div>
            <button
              onClick={onClose}
            ...
            {loading ? (
              ...
              <div className="space-y-4 relative">
                ...
                {history.map((record, index) => {
                  ...
                  if (olderRecord) {
                    if (record.overall_score > olderRecord.overall_score) trend = 'up';
                    else if (record.overall_score < olderRecord.overall_score) trend = 'down';
                  }

                  let feedbackObj = null;
                  try {
                    if (record.feedback) {
                      feedbackObj = JSON.parse(record.feedback);
                    }
                  } catch (e) {
                     // ignore
                  }

                  const formattedDate = record.created_at ? new Date(record.created_at).toLocaleDateString() : `Interview #${history.length - index}`;
                      ...
                      <div className="flex-1 bg-white border border-gray-100 shadow-sm rounded-xl p-4">
                        ...
                        {feedbackObj && (
                          <div className="text-sm text-gray-600 space-y-1">
                            {feedbackObj.review && (
                              <p className="line-clamp-2">{feedbackObj.review}</p>
                            )}
                          </div>
                        )}
                      </div>
